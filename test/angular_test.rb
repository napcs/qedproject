...
class AngularTest < ProjectTestCase
  ...
  def test_loads_angular_into_asset_file_when_jammit_specified
    p = QEDProject::Project.new(@folder, :libs => [:angular], :jammit => true)
    p.generate
  ...
  def test_loads_jqery_into_index_page_when_specified_along_with_angular
    p = QEDProject::Project.new(@folder, :libs => [:angular, :jquery])
    p.generate
    source = Pathname.new(File.join(@folder, "public", "index.html")).read
  ...
  def test_loads_jquery_into_asset_file_when_jammit_specified_along_with_angular
    p = QEDProject::Project.new(@folder, :libs => [:angular, :jquery], :jammit => true)
    p.generate
    source = Pathname.new(File.join(@folder, "config", "assets.yml")).read
    QEDProject::Libraries::JQuery.js_files.each do |js|
      assert source.include?("- javascripts/#{js}")
